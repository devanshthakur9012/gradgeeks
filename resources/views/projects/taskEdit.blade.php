@if ($project && $currentWorkspace && $task)
    <form class="needs-validation" novalidate method="post"
        action="@auth('web'){{ route('tasks.update', [$currentWorkspace->slug, $project->id, $task->id]) }}@elseauth{{ route('client.tasks.update', [$currentWorkspace->slug, $project->id, $task->id]) }}@endauth">
        @csrf
        <div class="modal-body">
            <div class="row">
                @if ($currentWorkspace->is_chagpt_enable())
                    <div class="text-end col-12">
                        <a href="#" data-size="lg" data-ajax-popup-over="true" class="btn btn-sm btn-primary"
                            data-url="{{ route('generate', ['task']) }}" data-bs-toggle="tooltip"
                            data-bs-placement="top" title="{{ __('Generate with AI') }}"
                            data-title="{{ __('Generate Task Title & Description') }}">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select class="form-control form-control-light" name="milestone_id" id="task-milestone" required>
                        ...
                        @foreach ($project->milestones as $milestone)
                            <option value="{{ $milestone->id }}" @if ($task->milestone_id == $milestone->id) selected @endif>
                                {{ $milestone->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-8">
                    <label class="col-form-label">{{ __('Title') }}</label><x-required></x-required>
                ...
                <div class="form-group col-md-4">
                    <label class="col-form-label">{{ __('Priority') }}</label><x-required></x-required>
                    <select class="form-control form-control-light" name="priority" id="task-priority" required>
                        <option value="Low" @if ($task->priority == 'Low') selected @endif>{{ __('Low') }}
                        </option>
                        <option value="Medium" @if ($task->priority == 'Medium') selected @endif>{{ __('Medium') }}
                        </option>
                        <option value="High" @if ($task->priority == 'High') selected @endif>{{ __('High') }}
                        </option>
                    </select>
                </div>
                <div class="form-group col-md-12">
                    <label class="col-form-label">{{ __('Assign To') }}</label><x-required></x-required>
                    <select class="multi-select" multiple="multiple" id="assign_to" name="assign_to[]"
                        data-toggle="select2" data-placeholder="{{ __('Select Users ...') }}" required>
                        @foreach ($users as $u)
                ...
                <div class="col-md-6">
                    ...
                    <div class='input-group form-group'>
                        <input type='text' class=" form-control pc-daterangepicker-3" id="duration" name="duration"
                            value="{{ __('Select Date Range') }}" placeholder="Select date range" required />
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control form-control-light" name="type" id="task-type" required>
                        ...
                        <option value="bussiness_analyst"  @if ($task->type == 'bussiness_analyst') selected @endif>{{ __('Bussiness Analyst') }}</option>
                        <option value="developer"  @if ($task->type == 'developer') selected @endif>{{ __('Developer') }}</option>
                        <option value="tester"  @if ($task->type == 'tester') selected @endif>{{ __('Tester') }}</option>
                    </select>
                </div>

                <div class="form-group">
                    <label class="col-form-label">{{ __('Description') }}</label><x-required></x-required>
                    <textarea class="form-control form-control-light" id="task-description" rows="3" required name="description">{{ $task->description }}</textarea>
                </div>
            </div>
        </div>
