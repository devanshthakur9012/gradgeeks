    ...
    <form method="post" class="needs-validation" novalidate
        ...
        <div class="modal-body">
            <div class="row">
                @if ($currentWorkspace->is_chagpt_enable())
                    <div class="text-end col-12">
                        <a href="#" data-size="lg" data-ajax-popup-over="true" class="btn btn-sm btn-primary"
                            data-url="{{ route('generate', ['milestone']) }}" data-bs-toggle="tooltip"
                            data-bs-placement="top" title="{{ __('Generate with AI') }}"
                            data-title="{{ __('Generate Milestone Title & Summary') }}">
                ...
                <div class="col-md-8">
                    <div class="form-group">
                        <label for="milestone-title"
                            class="col-form-label">{{ __('Milestone Title') }}</label><x-required></x-required>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="milestone-status"
                            class="col-form-label">{{ __('Status') }}</label><x-required></x-required>
                        <select class="form-control" name="status" id="milestone-status" required>
            ...
            <div class="form-group col-md-12 d-none">
                ...
                <div class="form-icon-user">
                    ...
                    <input type="number" class="form-control form-control-light currency_input" id="milestone-title"
                        placeholder="{{ __('Enter Cost') }}" value="{{ $milestone->cost }}" min="0"
                        name="cost" required>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group ">
                        <label class="form-label">{{ __('Start Date') }}</label><x-required></x-required>
                        <div class="input-group date ">
                            <input class="form-control datepicker22" type="text" id="start_date" name="start_date"
                                value="{{ $milestone->start_date }}" autocomplete="off" required>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="form-label">{{ __('End Date') }}</label><x-required></x-required>
                        <div class="input-group date ">
                            <input class="form-control datepicker23" type="text" id="end_date" name="end_date"
                                value="{{ $milestone->end_date }}" autocomplete="off" required>
                            <span class="input-group-text">
                                <i class="feather icon-calendar"></i>
                            </span>
                        </div>
            ...
            <div class="form-group col-md-12">
                <label for="task-summary" class="col-form-label">{{ __('Summary') }}</label><x-required></x-required>
                <textarea class="form-control form-control-light" id="task-summary" rows="3" name="summary" required>{{ $milestone->summary }}</textarea>
            </div>
