...
<script>
    ...
    brick.showPaymentForm(function(data) {
        if (data.flag == 1) {
            window.location.href = '{{ route('invoice.callback.error', [1, '_slug', $invoice_id]) }}'.replace(
                '_slug', data.slug);
        } else {
            window.location.href = '{{ route('invoice.callback.error', [2, '_slug', $invoice_id]) }}'.replace(
                '_slug', data.slug);
        }
    }, function(errors) {
        if (errors.flag == 1) {
