...
class User extends Authenticatable implements MustVerifyEmail
{
        ...
        if (!empty($workspace_id)) {
            ...
            $count = UserWorkspace::join('workspaces', 'workspaces.id', '=', 'user_workspaces.workspace_id')
                ->join('users', function ($join) {
                    $join->on('users.id', '=', 'workspaces.created_by')
                        ->where('users.id', '=', $this->id);
                })
                ->where('user_workspaces.permission', 'Member')
                ->get();
        }
    ...
    public function notifications($workspace_id)
    {
        return Notification::select('notifications.*')
            ->with(['user', 'project', 'task', 'bugReport'])
            ->leftJoin('projects', function ($join) {
                $join->on('notifications.related_id', '=', 'projects.id')
                    ->where('notifications.type', 'project_assign');
            })
            ->leftJoin('tasks', function ($join) {
                $join->on('notifications.related_id', '=', 'tasks.id')
                    ->where('notifications.type', 'task_assign');
            })
            ->leftJoin('bug_reports', function ($join) {
                $join->on('notifications.related_id', '=', 'bug_reports.id')
                    ->where('notifications.type', 'bug_assign');
            })
